...
T = [];
for i = 1:num_target
    t = TargetObj(i, [x{i}(1)-0.25; x{i}(1)-0.25; x{i}(1)+0.25; x{i}(1)+0.25] / 3, [y{i}(1)-0.25; y{i}(1)+0.25; y{i}(1)-0.25; y{i}(1)+0.25] / 3, theta{i}(1), 0, 0, 0);
    T = [T, t];
end
...
num_robot = 4;
for j = 1:4
    R = [R, Robot(j, [rand * 20/3, rand*20/3, rand * 2 * pi, 0], [0, 0], num_target)];
end

%% run three different control policy

[p_t1, c_t1, p1, p_r1, e1] = dec_fusion(A, W, T, num_target, R, num_robot, num_data, dt);
[p_t2, c_t2, p2, p_r2, e2] = optimal_GP(A, W, T, num_target, R, num_robot, num_data, dt);
[p_t3, c_t3, p3, p_r3, e3] = no_fusion(A, W, T, num_target, R, num_robot, num_data, dt);

%% plot result

plot_error({p1, p2, p3}, {p_t1, p_t2, p_t3}, num_data, num_target, num_robot, 5)
